...
tags:
  - graphs
  - Databases
title: "Neo4j: Export and Reload (updated)"
date: 2025-12-03
layout: post
teaser: Neo4j has opinions about how data is handled...
original: 2025-10-01
---
...
| CSV <br>(one file) | Export easy, import hard.<br>Requires pre-processing to do import.<br>If import/pre-processing is done wrong, silently drops data. |
...
My go to (in December 2025) is the JSON option:  It is fast-enough and has the fewest pitfalls, but be aware that JSON import can take an order of magnitude longer than export.
## Binary export
...
- If you don't do it right can *silently* drop of data on import

**Conclusion:** The post-processing has been a little tricky.  This is a flexible and faster-than-CSV option if you are comfortable making it.
...
**Note:** The same trick used in JSON for many-node-labels might work here to. I ran out of time to investigate!
## JSON export
**Export**:
```cypher
